...
export default function PointsSidebar({ points, onFly, onDelete, routeMode, routeFrom, onRouteToggle }) {
  const { user } = useAuth()

  const getUserInfo = (userId) => USERS.find(u => u.id === userId) || { name: userId, color: '#8888aa', avatar: '👤' }
        ...
        : points.map(p => {
            ...
            return (
              <div key={p.id}
                ...
                <div className="point-badge" style={{ background: t.color + '22' }}>
                  {t.emoji}
                </div>
                ...
                <div className="point-info">
                  ...
                  {p.comment && <div className="point-comment">"{p.comment}"</div>}
                </div>

                {(p.created_by === user?.id) && (
                  <button className="point-del"
