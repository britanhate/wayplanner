...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    mapInstance.current = L.map(mapRef.current, { zoomControl: true }).setView([48.8566, 2.3522], 12)
  ...
  const handleRoutePointSelect = async (p) => {
    if (routeStep === 1) {
      ...
      try {
        ...
        setRouteResult({
          from: routeFrom, to: p,
          totalMin: res.totalMin, totalKm: res.totalKm,
  ...
  const drawRoute = (paths) => {
    clearRouteLines()
    paths.forEach(path => {
      const l = L.polyline(path.map(c => [c[1], c[0]]), { color: '#2a7de8', weight: 4, opacity: .8 })
        .addTo(mapInstance.current)
      routeLayers.current.push(l)
    })
    if (routeLayers.current.length)
      mapInstance.current.fitBounds(L.featureGroup(routeLayers.current).getBounds().pad(.15))
  }
  ...
  return (
    <div className="map-view">
      <div className="map-sidebar">
        ...
        <div className="sidebar-section">
          <button
            ...
            onClick={routeMode ? () => { setRouteMode(false); setRouteStep(0); setRouteFrom(null) } : startRouteMode}>
            {routeBuilding ? '⏳ Будуємо...' : routeMode ? `🔴 ${routeStep === 1 ? 'Оберіть старт' : 'Оберіть фініш'}` : '🚌 Маршрут транспортом'}
          </button>
        </div>
      ...
      <div className="map-wrap">
        ...
        {routeResult && (
          <RoutePanel result={routeResult} onClose={() => { setRouteResult(null); clearRouteLines() }} />
        )}
      </div>
      ...
      {pendingPos && (
        <AddPointModal
          ...
          onClose={() => { setPendingPos(null); setGeocoded(null) }}
        />
      )}
    </div>
  )
